...
export default function SupplyPlanner() {
  ...
  const [policy, setPolicy] = useState(null);   // standing reorder policy (light state read)
  ...
  useEffect(() => {
    ...
    api.optimization.policy()
      .then((d) => { if (alive) setPolicy(d.state); })
      .catch(() => {});
    return () => { alive = false; };
  }, []);
  ...
  return (
    <div className="content">
      ...
      {/* Standing policy chip — the evidence and the choice live on Optimization */}
      <div className="card" style={{ marginTop: 16, borderColor: '#99f6e4' }}>
        <div className="card-body" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
          <div style={{ minWidth: 0 }}>
            <div style={{ fontSize: 12, color: C.muted, textTransform: 'uppercase', letterSpacing: 0.6 }}>Standing reorder policy</div>
            <div style={{ fontSize: 16, fontWeight: 700, color: C.teal, marginTop: 2 }}>
              {policy?.label || 'Forecast base-stock'}
              {policy?.tuned_at && <span className="tag tag-success" style={{ fontSize: 10, marginLeft: 8 }}>tuned</span>}
            </div>
            <div style={{ fontSize: 12, color: C.muted, marginTop: 2 }}>
              Policy evidence, adoption and parameter tuning live on the Optimization page.
            </div>
          </div>
        </div>
      </div>
