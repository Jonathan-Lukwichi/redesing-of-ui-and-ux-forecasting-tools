    ...
    <div className="content" style={{ display: 'flex', flexDirection: 'column', gap: 22 }}>
      ...
      <StartBar
        ...
        summary={specialty && alias
          ? `${specialty} · ${alias} · ${horizons.find((h) => h.id === horizon)?.label || horizon}`
      ...
      {/* Step 4 — Forecast window (derived, weather-app style) */}
      {selectedSpecialty && (
        <Section step="4" title="Forecast window" sub="Begins right after this specialty's latest data.">
          <div style={{
            ...
            <span>
              Forecasts <strong>{horizonCount(horizon)} {isWeekly ? 'week' : 'day'}{horizonCount(horizon) > 1 ? 's' : ''}</strong> forward
              from <strong>{specialty}</strong>'s most recent {isWeekly ? 'week' : 'day'}. Tap any card for detail.
            </span>
          </div>
        </Section>
      )}
