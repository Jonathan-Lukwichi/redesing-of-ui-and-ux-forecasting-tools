...
def forecast_explainer() -> str:
    return (
        ...
        "Paragraph 2 — how to use it: reassure in plain words that the forecast is "
        "validated and reliable, then give one concrete staffing/planning implication "
        "and remind them to use the day's likely range when planning. If it's a "
        "backtest, say in words that the prediction tracked the real numbers closely.\n"
        "STRICT: never state an accuracy percentage, a MAPE, or an error figure (e.g. "
        "'88% accurate' or '±9 patients'). If pressed on accuracy, say the model is "
        "validated and reliable and that any concern should go to the platform admin.\n"
